...
<body class="bg-zinc-950 font-sans text-white antialiased">
    ...
    <div class="pointer-events-none fixed inset-0 -z-10 overflow-hidden">
        <div class="absolute -left-48 -top-48 size-[500px] rounded-full bg-teal-500/8 blur-[140px]"></div>
        <div class="absolute -bottom-48 right-1/4 size-[400px] rounded-full bg-amber-400/6 blur-[120px]"></div>
    </div>
    ...
    <div class="mx-auto max-w-5xl px-6 py-12 lg:px-10">
        ...
        {{-- Success message --}}
        @if (session('success'))
            <div class="mb-8 rounded-2xl border border-teal-800/40 bg-teal-900/20 p-6 text-center backdrop-blur-sm">
                <div class="mx-auto mb-3 flex size-14 items-center justify-center rounded-full bg-teal-500/20">
                    <svg class="size-7 text-teal-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
                </div>
                <h2 class="text-lg font-semibold text-teal-300">Quotation Request Submitted!</h2>
                <p class="mt-1 text-sm text-zinc-400">{{ session('success') }}</p>
                <a href="{{ route('fabrics.index') }}" class="mt-4 inline-flex items-center gap-1.5 text-sm font-medium text-teal-400 transition hover:text-teal-300">
                        ...
                        <div class="p-5">
                            ...
                            @if ($fabric->color || $fabric->roll_code)
                                <div class="mt-2 flex flex-wrap gap-3">
                                    ...
                                    @endif
                                    @if ($fabric->roll_code)
                                        <span class="font-mono text-[11px] text-zinc-600">{{ strtoupper($fabric->roll_code) }}</span>
                                    @endif
                                </div>
                            @endif
            ...
            <div class="lg:col-span-3">
                <div class="rounded-2xl border border-zinc-800/60 bg-zinc-900/40 p-6 backdrop-blur-sm sm:p-8">
                    ...
                    <form method="POST" action="{{ route('fabrics.submit') }}" class="mt-6 space-y-5" x-data="{
                        ...
                        <div class="grid gap-5 sm:grid-cols-2">
                            <div>
                                ...
                                <input type="text" name="customer_name" value="{{ old('customer_name') }}" required
                                    class="w-full rounded-xl border border-zinc-700/60 bg-zinc-800/40 px-4 py-2.5 text-sm text-white placeholder-zinc-500 outline-none transition focus:border-teal-500/50 focus:bg-zinc-800/80 focus:ring-2 focus:ring-teal-500/20"
                        ...
                        <div class="border-t border-zinc-800/50 pt-5">
                            ...
                            <div class="mt-4 grid gap-5 sm:grid-cols-2">
                                <div>
                                    ...
                                    <input type="number" name="length_meters" x-model="length" step="0.01" min="0.01" required
                                        class="price-input w-full rounded-xl border border-zinc-700/60 bg-zinc-800/40 px-4 py-2.5 text-sm text-white placeholder-zinc-500 outline-none transition focus:border-teal-500/50 focus:bg-zinc-800/80 focus:ring-2 focus:ring-teal-500/20"
                                        placeholder="e.g. 5" />
                            ...
                            <div class="mt-5 rounded-xl border border-zinc-700/40 bg-zinc-800/30 p-4">
                                <div class="flex items-center justify-between text-sm">
                                    ...
                                    <span class="text-lg font-bold text-teal-400" x-text="'UGX ' + total.toLocaleString('en-US', {minimumFractionDigits: 0, maximumFractionDigits: 0})">UGX 0</span>
                                </div>
                        ...
                        <button type="submit"
                            class="w-full rounded-xl bg-gradient-to-r from-teal-600 to-teal-500 px-6 py-3 text-sm font-semibold text-white shadow-lg shadow-teal-600/20 transition hover:from-teal-500 hover:to-teal-400 hover:shadow-teal-500/30">
    ...
    <footer class="border-t border-zinc-800/50 px-6 py-6 lg:px-10">
        <div class="mx-auto flex max-w-5xl items-center justify-between">
            <div class="flex items-center gap-3">
                <img src="{{ asset('logos/logo-white.png') }}" alt="{{ config('app.name', 'Akatabo') }}" class="h-5 w-auto opacity-50">
